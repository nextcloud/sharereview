OC.L10N.register(
    "sharereview",
    {
    "Delete" : "O'chirish",
    "Are you sure?" : "Ishonchingiz komilmi?",
    "The share will be deleted!" : "Ulanish o'chiriladi!",
    "All Shares" : "Barcha ulashmalar",
    "New Shares" : "Yangi ulashmalar",
    "Confirm reviewed" : "Tekshiruvni tasdiqlang",
    "Reset time" : "Vaqtni tiklash",
    "Export report" : "Eksport hisoboti",
    "Show talk shares" : "Baham ko'rish",
    "Select folder" : "Jildni tanlang",
    "Share deleted" : "Ulashish oʻchirildi",
    "Table not reloaded" : "Jadval qayta yuklanmadi",
    "Request could not be processed" : "So‘rovni ko‘rib bo‘lmadi",
    "Timestamp saved" : "Vaqt tamg‘asi saqlandi",
    "Timestamp deleted" : "Vaqt tamg‘asi o‘chirildi",
    "Report created" : "Hisobot yaratildi",
    "Settings saved" : "Settings saved",
    "OK" : "Yaxshi",
    "Cancel" : "Bekor qilish",
    "Search" : "Qidirish",
    "Show _MENU_ entries" : "_MENU_ yozuvlarini ko'rsatish",
    "Showing _START_ to _END_ of _TOTAL_ entries" : "_TOTAL_ ta yozuvdan _START_ dan _END_ gacha koʻrsatilmoqda",
    "Showing 0 to 0 of 0 entries" : "0 ta yozuvdan 0 dan 0 gacha koʻrsatilmoqda",
    "App" : "Ilova",
    "Item" : "Mahsulot",
    "Initiator" : "Boshlovchi",
    "Type" : "Turi",
    "Permissions" : "Ruxsatnomalar",
    "Time" : "Vaqt",
    "Action" : "faoliyat",
    "Select all" : "Barchasini tanash",
    "more" : "Ko'proq",
    "read" : "o'qish",
    "edit" : "tahrirlash",
    "Password protected" : "Parol bilan himoyalangan",
    "Expiration date: " : "Tugash muddati:",
    "Deck" : "Pastki qavat",
    "Talk room" : "Suhbat xonasi",
    "Team" : "Jamoa",
    "Federation" : "Jamg'arma",
    "E-mail" : "E-mail",
    "Link" : "Havola",
    "User group" : "Foydalanuvchilar guruhi",
    "User" : "Foydalanuvchi",
    "Group" : "Guruh",
    "Other" : "Boshqa",
    "Files" : "Fayllar",
    "Share Review" : "Shahr ulashish",
    "Review and audit all shares" : "Barcha ulashmalarni ko'rib chiqing va tekshiring",
    "This app can be used for auditing shares within a Nextcloud instance e.g. for data loss prevention.\nShare status can be exported for documentation.\n\n- Review any file share\n  - files, talk, deck, teams, federation\n  - app specific shares (if implemented by other apps)\n- Remove shares\n- Confirm current review\n  - show only new shares next time\n- Audit compliance\n- Assign review to user groups (e.g. audit or risk mgmt)\n- Export as CSV or PDF (manual or regular background job)\n\nNote:\nThe app must be restricted to at least one specific user group in the app store.\nThis prevents accidental exposure of the shared content to all users." : "Ushbu ilovadan Nextcloud nusxasi ichidagi ulashishlarni audit qilish, masalan, ma'lumotlar yo'qotilishining oldini olish uchun foydalanish mumkin.\n\nUlashish holatini hujjatlar uchun eksport qilish mumkin.\n\n- Har qanday fayl ulashishni ko'rib chiqish\n- fayllar, suhbat, desk, jamoalar, federatsiya\n- ilovaga xos ulashishlar (agar boshqa ilovalar tomonidan amalga oshirilsa)\n- Ulashishlarni olib tashlash\n- Joriy sharhni tasdiqlash\n- keyingi safar faqat yangi ulashishlarni ko'rsatish\n- Muvofiqlikni tekshirish\n- Sharhni foydalanuvchi guruhlariga tayinlash (masalan, audit yoki xavfni boshqarish)\n- CSV yoki PDF sifatida eksport qilish (qo'lda yoki oddiy fon ishi)\n\nEslatma:\nIlova ilovalar do'konida kamida bitta ma'lum foydalanuvchi guruhi bilan cheklangan bo'lishi kerak.\nBu ulashilgan kontentning barcha foydalanuvchilarga tasodifiy ta'sir qilishining oldini oladi.",
    "(*) indicates incorrect data. Share should be removed after evaluation." : "(*) noto'g'ri ma'lumotlarni bildiradi. Baholashdan keyin ulush olib tashlanishi kerak.",
    "Delete selected" : "Tanlangan oʻchirish",
    "Pause reload after deletion" : "O'chirishdan keyin qayta yuklashni to'xtatib turing",
    "No share found" : "Hech qanday ulashish topilmadi",
    "Shares are being retrieved, please wait …" : "Ulanishlar olinmoqda, kuting…",
    "The app must be restricted to at least one specific user group in the app store. This prevents accidental exposure of the shared content to all users." : "Ilova ilovalar do'konidagi kamida bitta foydalanuvchi guruhi bilan cheklangan bo'lishi kerak. Bu barcha foydalanuvchilarga umumiy tarkibning tasodifiy ta'sirini oldini oladi.",
    "Click here" : "Bu yerga bosing",
    "On demand report" : "Talab bo'yicha hisobot",
    "CSV" : "CSV",
    "PDF" : "PDF",
    "Scheduled report" : "Rejalashtirilgan hisobot",
    "Default folder" : "Standart jild",
    "Folder owner" : "Jild egasi",
    "Schedule" : "Schedule",
    "None" : "Yo'q",
    "Daily" : "Kunlik",
    "Weekly" : "Haftalik",
    "Monthly" : "Oylik",
    "Format" : "Formatlash",
    "Save" : "Saqlash"
},
"nplurals=1; plural=0;");
